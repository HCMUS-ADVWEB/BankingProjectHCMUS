...
const employeeReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SET_DEPOSIT_ACCOUNT':
      return {
        ...state,
        depositAccount: { ...state.depositAccount, ...action.payload },
      };
    case 'SET_TRANSACTION_ACCOUNT_HISTORY':
      return {
        ...state,
        transactionAccountHistory: {
          ...state.transactionAccountHistory,
          ...action.payload,
        },
      };
...
export const EmployeeProvider = ({ children }) => {
  ...
  const getFormattedDepositAmount = useCallback(
    () => formatVND(state.depositAccount.amount),
    [state.depositAccount.amount],
  );
  ...
  const getFormattedTransactionAmount = useCallback(
    () => formatVND(state.transactionAccountHistory.amount),
    [state.transactionAccountHistory.amount],
  );
    ...
    try {
      ...
      if (res && Array.isArray(res.data)) {
        ...
        dispatch({
          ...
          payload: res.data.accounts || [res.data],
        });
      } else {
      ...
      dispatch({
        type: 'SET_SUCCESS',
        payload: 'Accounts loaded successfully!',
      });
      ...
      dispatch({
        type: 'SET_ERROR',
        payload: err.message || 'Failed to fetch accounts',
      });
      throw err;
    } finally {
  ...
  const handleCreateAccount = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: 'SET_ERROR',
        payload: err.message || 'Create account failed',
      });
      throw err;
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
  ...
  const fetchTransactions = useCallback(async (customerId, params = {}) => {
    ...
    try {
      ...
      let txs = [];
      if (res && res.data) {
        const sender = Array.isArray(res.data.transactionsAsSender)
          ? res.data.transactionsAsSender
          : [];
        const receiver = Array.isArray(res.data.transactionsAsReceiver)
          ? res.data.transactionsAsReceiver
          : [];
        txs = [...sender, ...receiver];
      ...
      dispatch({
        type: 'SET_SUCCESS',
        payload: 'Transactions loaded successfully!',
      });
      setTimeout(() => dispatch({ type: 'CLEAR_SUCCESS' }), 3000);
      return txs;
    } catch (err) {
      dispatch({
        type: 'SET_ERROR',
        payload: err.message || 'Fetch transactions failed',
      });
      throw err;
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
